...
<html>
	<head>
		...
		<meta property="og:url"           content="https://www.wikighana.com/add-customer.php" />
		<meta property="og:type"          content="website" />
		<meta property="og:title"         content="Add Customer - Wiki Ghana" />
		<meta property="og:description"   content="Ghana News, Dating, Buy/Sell" />
		<meta property="og:image"  prop="image"       content="https://wikighana.com/img/icons/favicon2.jpg" />
		...
		<div class="wrap">
        		...
        		<?php include($_SERVER['DOCUMENT_ROOT']."/include/header.php");
                    ...
                    if(isset($_POST["submit_update"])){
                        $_GET['customer_id'] = $_SESSION['customer_id'];
                        $_GET['page'] = $_SESSION['page'];
                        
                    }


                    if(isset($_GET["customer_id"], $_GET["is_approved"])){
                        if($_GET["is_approved"] == "true"){
                            $query = "UPDATE `customers` SET `is_approved` = '1' WHERE `id` = '{$_GET['customer_id']}' ";
                        }elseif($_GET["is_approved"] == "false"){
                            $query = "UPDATE `shops` SET `is_approved` = '0' WHERE `id` = '{$_GET['customer_id']}' ";
                        }
                    ...
                    // if(isset($_GET["customer_id"], $_GET["page"],$_GET["remove"])){
                    //     $customer_id = $_GET["shop_id"];
                    ...
                    if(isset($_GET["shop_id"], $_GET["page"],$_GET["del"])){
                        ...
                        $query = "DELETE FROM `shops` WHERE `shops`.`id` = $customer_id LIMIT 1;";
                        $run = mysqli_query($connection, $query);
                        if(mysqli_affected_rows($connection) > 0){
                            //$output .= "<p style='color:green;'><strong>Photo Removed !!</strong></p>";
                            die(header("Location: /add-customer.php?page=$page"));
                        }else{$output .= "<p style='color:red;'><strong>Photo Not Removed !!</strong></p>";}
                    }
                	...
                	if(isset($_POST["submit_insert"]) || isset($_POST["submit_update"])){
                        ...
                        $customer_full_name = mysqli_real_escape_string($connection, trim($_POST["customer_full_name"]));
                    ...
                    if(isset($_GET["customer_id"])){
                        ...
                        if(mysqli_num_rows($run) > 0){
                            ...
                            $customer_full_name_c = $row['customer_full_name'];
...
$query4 = "SELECT Count(*) AS `count_records` FROM `customers`";
...
// $query = "SELECT `id`,`item_name`,`img_1` FROM `customer_item` WHERE `customer_id` = '$customer_id' ORDER BY `id` LIMIT {$rec_start}, 1";
// $run = mysqli_query($connection, $query);
// if(mysqli_num_rows($run) > 0){
...
$query = "SELECT `id`,`customer_full_name`,`logo_url`,`is_approved` FROM `customers`  ORDER BY `id` DESC LIMIT {$rec_start},  {$perPage}";
$run = mysqli_query($connection, $query);
if(mysqli_num_rows($run) > 0){
...
?>
        <tr>
            <td style="border-left:5px solid white" <?php if(isset($_GET["customer_id"])){if($_GET["customer_id"] == $customer_id){echo "class='selected'";}} ?> ><a style="color:black;text-decoration:none;" href="/add-customer.php?customer_id=<?php echo $customer_id.'&page='.$page; ?>"><?php echo $customer_id; ?></a></td>
            <td><a style="color:blue;text-decoration:none;" href="/add-customer.php?customer_id=<?php echo $customer_id.'&page='.$page; ?>"><?php echo $customer_full_name; ?></a></td>
            <td><a style="text-decoration:none;" href="/add-customer.php?customer_id=<?php echo $customer_id.'&page='.$page; ?>"><img style="width:50px; height:50px;vertical-align:middle;" src="<?php echo $customer_img; ?>"></a><?php echo $how_tick_mark; ?></td>
        </tr>
<?php
...
?>
                    ...
                    <div class="productContainerRight">
                        <div class="container">
                            ...
                            <div class="top_heading">
                                ...
                                if(isset($_GET["customer_id"])){
                                    if($is_approved == 0){
                                        echo "<a href='/add-customer.php?customer_id={$_GET['customer_id']}&is_approved=true&page={$page}'>Approve</a>";
                                    }elseif($is_approved == 1){
                                        echo "<a href='/add-customer.php?customer_id={$_GET['customer_id']}&is_approved=false&page={$page}'>Revoke</a>";
                                    }
                                }
                                ?>   
                            </div> 

                            <form action="add-customer.php" enctype="multipart/form-data"  method="post">
                                <?php echo $output; ?>
                                <table>
                                    ...
                                    <tr>
                                        <td>
                                            <label for="customer_name"><b>Customer Full Name <span style="color:red">*</span></b></label>
                                        </td>
                                        <td>
                                            <input required type="text" maxlength="200" name="customer_name" id="customer_name" <?php if(isset($customer_name_c)){echo "value='{$customer_name_c}'";} ?> >
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input placeholder="Enter Email Address" required type="text" maxlength="200" name="email" id="email" <?php if(isset($email_c)){echo "value='{$email_c}'";} ?> >
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td>
                                            <label for="op_balance_credit"><b>OP Bal. Credit</b></label>
                                        </td>
                                        <td>
                                            <input required type="number" maxlength="0" name="op_balance_credit" id="op_balance_credit" <?php if(isset($op_balance_credit_c)){echo "value='{$op_balance_credit_c}'";} ?> >
                                        </td>
                                    </tr>

                                    <tr>
                                        <td>
                                            <label for="op_balance_debit"><b>OP Bal. Debit</b></label>
                                        </td>
                                        <td>
                                            <input required type="number" maxlength="0" name="op_balance_debit" id="op_balance_debot" <?php if(isset($op_balance_debit_c)){echo "value='{$op_balance_debit_c}'";} ?> >
                                        </td>
                                    </tr>
                    ...
                    <br>
                                <a href="/add-customer.php?page=<?php echo $page;?>"><button type="button">Add New</button></a>
                               
                                
                                    <button class="btnSubmit" type="submit" name="<?php if(isset($_GET['customer_id'])){echo 'submit_update';}else{echo 'submit_insert';} ?>" id="submit_insert">Save</button>

                                    <?php
                                    if(isset($_GET["customer_id"])){
                                      
                                        echo "<a style='float:right' href='/add-customer.php?customer_id={$_GET['customer_id']}&page={$page}&del=true'><button type='button'>Delete</button></a>";
                                    }
				...
				<?php 
				 if(isset($_GET['customer_id'], $_GET['page'])){
