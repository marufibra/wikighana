...
<html>
	<head>
		...
		<title>News In Ghana - Wiki Ghana</title>
	...
	<body>
		<div class="wrap">
			...
			<div class="content_flex">
				<div class="content">
				<div class="site_nav"><a href="/">Home</a> <i class="bi bi-arrow-right"></i> <a href="/news.php">News</a></div>
					<!-- <div class="site_nav"><a href="/">Home</a> <i class="bi bi-arrow-right"></i> <a href="news.php">News</a></div> -->
					...
					<?php
						...
						$query = "SELECT `news_content`.`content_id`,`news_content`.`subject`,`news_content`.`publish_date_time`,`img_id`,`is_video_attached` FROM `news_content`  WHERE (publish_date_time = (SELECT MAX(`publish_date_time`) AS publish_date FROM `news_content` WHERE headline = 1 AND news_or_article = 1 AND `status`=1))";
					...
					<div class="category_name" id="top_story_58956" style="margin:0 0 10px 0;padding-left:10px;">Top Stories</div>
					<?php
					...
					<div class="hide_mobile_ad" style="margin:0 auto;width:728px">
					...
					<?php
						...
						$query3 = "SELECT `news_content`.`content_id`,`news_content`.`subject`,`news_content`.`publish_date_time`,`img_id`,`is_video_attached` FROM `news_content`  WHERE `content_id` not in ($str_top_story_headline)  AND news_or_article = 1  AND `status`=1 ORDER BY `publish_date_time` DESC LIMIT {$rec_start},  {$perPage}";
						...
						while($row = mysqli_fetch_assoc($run3)){
								...
								$output = ("<div class='wrapper_news_category' title='{$subject}'>
									...
									<a href='{$file_path}' class='news_category_subject' >{$subject}</a>{$reference_point}
								...
								if( $page == 1){
									$counter++;
									if($counter == 20){
										$reference_point = "<span id='latest_news_191224'></span>";
						...
						echo '<div class="category_name" id="latest_news_58956"   style="padding-left:10px;margin-top:0 !important">Latest News</div>';
