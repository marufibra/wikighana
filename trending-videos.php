...
<html>
	<head>
		...
		<title>Trending Videos - Wiki Ghana</title>
		<link rel="canonical" href="/trending-videos.php">
	</head>
	<body>
		<div class="wrap">
			...
			<div class="content_flex">
				<div class="content">
					...
					<div class="site_nav"><a href="/">Home</a> <i class="bi bi-arrow-right"></i> <a href="/trending-videos.php">Trending Videos</a></div>
					
					
					<?php
					include($_SERVER['DOCUMENT_ROOT']."/include/advert_top.php");
					
					$query = "SELECT `news_content`.`content_id`,`news_content`.`subject`,`news_content`.`publish_date_time`,`img_id`,`is_video_attached` FROM `news_content`  WHERE (publish_date_time = (SELECT MAX(`publish_date_time`) AS publish_date FROM `news_content` WHERE category_headline = 1 AND (category_id=5 OR `is_video_attached`=1) AND `status`=1))";
					...
					<div class="category_name" style="margin:30px 0 10px 0;padding-left:10px">Top Stories (Trending Videos)</a></span></div>
					<?php
					
					$query = "SELECT `news_content`.`content_id`,`news_content`.`subject`,`news_content`.`publish_date_time`,`img_id`,`is_video_attached` FROM `news_content`  WHERE `category_top_story` = 1 AND `content_id` != {$content_id_headline} AND (`category_id`=5 OR `is_video_attached`=1) AND `status`=1 ORDER BY `publish_date_time` DESC LIMIT 4";
					...
					$query4 = "SELECT Count(*) AS `count_records` FROM `news_content`  WHERE `content_id` not in ($str_top_story_headline) AND news_or_article = 1 AND category_top_story = 1 AND ('category_id'=5 OR is_video_attached=1) AND `status`=1";// ORDER BY `publish_date_time` DESC LIMIT 102";
					...
					if($records > $perPage){
						...
						$query3 = "SELECT `news_content`.`content_id`,`news_content`.`subject`,`news_content`.`publish_date_time`,`img_id`,`is_video_attached` FROM `news_content`  WHERE `content_id` not in ($str_top_story_headline) AND news_or_article = 1 AND category_top_story = 1 AND ('category_id'=5 OR is_video_attached=1) AND `status`=1 ORDER BY `publish_date_time` DESC LIMIT {$rec_start},  {$perPage}";
						...
						echo '<div class="category_name" style="margin-top:0px;padding-left:10px" id="latest_news_58956">Latest Stories (Trending Videos)</div>';
